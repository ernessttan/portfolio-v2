...
function Footer() {
  return (
    <footer className="bg-offwhite p-5 py-8 dark:bg-lightblack dark:dark-text md:px-14">
      <NavLink to="/">
        <svg className="h-10 w-10 fill-black lg:w-14 lg:h-14 dark:fill-offwhite" viewBox="0 0 77 44" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="my-5 flex gap-5 md:gap-15 md:flex-row">
        <NavLink className="text-lg font-semibold decoration-4 hover:underline md:text-xl lg:text-2xl" to="/">Home</NavLink>
        <NavLink className="text-lg font-semibold decoration-4 hover:underline md:text-xl lg:text-2xl" to="/contact">Contact</NavLink>
        <a href="/Ernest Tan_Resume.pdf" className="text-lg font-semibold decoration-4 hover:underline md:text-xl lg:text-2xl" download>Resume</a>
      </div>
      <div className="my-5 flex items-center gap-5 md:py-0">
        <a target="_blank" href="https://twitter.com/ernessttan" rel="noreferrer">
          <Twitter className="h-6 w-6 md:h-7 md:w-7  dark:fill-offwhite" />
        </a>
        <a target="_blank" href="https://www.linkedin.com/in/ernesttan23/" rel="noreferrer">
          <Linkedin className="h-6 w-6 md:h-7 md:w-7  dark:fill-offwhite" />
        </a>
        <a target="_blank" href="https://github.com/ernessttan" rel="noreferrer">
          <GitHub className="h-6 w-6 md:h-7 md:w-7  dark:fill-offwhite" />
        </a>
        <a target="_blank" href="https://www.twitter.com" rel="noreferrer">
          <Mail className="h-6 w-6 md:h-7 md:w-7 dark:stroke-offwhite" />
        </a>
      </div>
      <div className="flex items-center gap-2 my-5">
        <GlobeIcon className="w-5 h-5 md:h-7 md:w-7" />
